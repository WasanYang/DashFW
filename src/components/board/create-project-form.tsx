...
export function CreateProjectForm({ clients, onSubmit, translations }: CreateProjectFormProps) {
    const [isSubmitting, setIsSubmitting] = useState(false);

    const FormSchema = z.object({
        title: z.string().min(1, translations.titleRequired),
        clientId: z.string().min(1, translations.clientRequired),
        gross_price: z.coerce.number().positive({ message: translations.priceRequired }),
        ...
        <Form {...form}>
            <form onSubmit={form.handleSubmit(handleFormSubmit)} className="space-y-4 py-4">
                        ...
                        <FormItem>
                            <FormLabel>{translations.clientLabel}</FormLabel>
                             <Select onValueChange={field.onChange} defaultValue={field.value}>
                                <FormControl>
                                    <SelectTrigger>
                                        <SelectValue placeholder={translations.selectClientPlaceholder} />
                                    </SelectTrigger>
                                </FormControl>
                                <SelectContent>
                                    {clients.map(client => (
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem className="flex-grow">
                                            <FormControl>
                                                <Input placeholder={`Sub-task ${index + 1}`} {...field} />
                                            </FormControl>
                                            <FormMessage/>
                                        </FormItem>
                                    )}
                ...
                <Button type="submit" disabled={isSubmitting} className="w-full">
                    ...
                    {isSubmitting ? translations.creatingProjectButton : translations.createProjectButton}
                </Button>
            </form>
        </Form>
    );
